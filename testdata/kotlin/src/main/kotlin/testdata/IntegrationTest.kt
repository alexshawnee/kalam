...
class MyUserService : UserServiceHandler {
    override suspend fun getUser(request: GetUserRequest): GetUserResponse {
        ...
        return GetUserResponse.newBuilder()
            .setName("User ${request.id}")
            .setEmail("user${request.id}@test.com")
            .build()
    }
...
fun main() = runBlocking {
    ...
    val r1 = UserService.getUser(GetUserRequest.newBuilder().setId(42).build())
    println("← name: ${r1.name}, email: ${r1.email}")
    check(r1.name == "User 42")

    // Test streaming
    println("→ ListUsers(query: \"all\")")
    val users = UserService.listUsers(ListUsersRequest.newBuilder().setQuery("all").build())
        .toList()
    for (user in users) {
